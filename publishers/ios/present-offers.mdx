...
> **⚠️ Important:** When presenting offers in critical flows (like cancellation), always ensure the user can proceed with their intended action. In the example above, `proceedWithCancellation()` is called for both rejection and error cases, ensuring users are never blocked from canceling their subscription.

**Why it's recommended:**
- ✅ Zero config to get started - works out of the box
- ✅ Optional callbacks for custom logic when needed
...
> **💡 Note:** The SDK is designed to never crash your app. All errors are delivered via callbacks, allowing you to handle them gracefully and always let users proceed with their intended action.
